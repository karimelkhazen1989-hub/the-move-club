...
const seed = {
  ...
  invoices: [
    {
      ...
      source: 'Membresías',
      emailStatus: 'Enviado',
      sentAt: '24/03/2026 09:15',
      pdfReady: true,
    },
    {
      id: 'F-0002',
      client: 'Nahuel Erices',
      email: 'nahuel@example.com',
      amount: 2500,
      status: 'Pendiente',
      cae: '-',
  ...
  costs: [
    { id: 'cst_1', type: 'Fijo', concept: 'Alquiler local', amount: 4200000 },
    {
      id: 'cst_2',
      type: 'Fijo',
      concept: 'Servicios (luz/gas/wifi)',
      amount: 280000,
    },
    {
      id: 'cst_3',
  ...
  sales: [
    {
      id: 'sal_1',
      unit: 'Membresías',
      concept: 'Plan mensual — Florencia',
      client: 'Florencia James',
      amount: 95000,
      method: 'Transferencia',
      createdAt: '28/03/2026 08:30',
    },
    {
      id: 'sal_2',
      unit: 'Bebidas',
      concept: 'Agua 500ml x2',
      client: 'Nahuel Erices',
      amount: 5000,
      method: 'Efectivo',
    ...
    },
    {
      id: 'sal_3',
      unit: 'Membresías',
      concept: 'Pase diario — Rodrigo',
      client: 'Rodrigo Pichot',
      amount: 15000,
      method: 'QR',
      createdAt: '28/03/2026 09:45',
    },
    {
      id: 'sal_4',
      unit: 'Suplementos',
      concept: 'Creatina 300g',
      client: 'Rodrigo Pichot',
      amount: 22000,
      method: 'Efectivo',
      createdAt: '28/03/2026 10:15',
    },
  ],
  activity: [
    {
      id: 'act_1',
      text: 'Sistema iniciado — The Move Club',
      createdAt: '28/03/2026 08:00',
    },
  ],
};
...
function computeTotals({ sales, costs, products, invoices, members }) {
  ...
  const byUnit = {};
  sales.forEach((s) => {
    byUnit[s.unit] = (byUnit[s.unit] || 0) + Number(s.amount || 0);
  });
  return {
    revenue,
    totalCosts,
...
const api = {
  ...
  async getMembers(q = '') {
    await wait();
    return readStore().members.filter((m) =>
  ...
  async saveMember(f) {
    ...
    s.activity = [
      {
        id: `act_${Date.now()}`,
        text: `${f.id ? 'Socio actualizado' : 'Nuevo socio'}: ${f.name}`,
        createdAt: now(),
      },
      ...s.activity,
    ].slice(0, 20);
  ...
  async saveSale(f) {
    ...
    const sale = {
      ...
      method: f.method || 'Efectivo',
      createdAt: now(),
    };
    const inv = {
      id: `F-${String(s.invoices.length + 1).padStart(4, '0')}`,
      client: f.client,
      email: f.email || '',
      amount: Number(f.amount),
      status: 'Pendiente',
    ...
    s.sales = [sale, ...s.sales];
    s.invoices = [inv, ...s.invoices];
    s.activity = [
      {
        id: `act_${Date.now()}`,
        text: `Venta: ${f.concept}`,
        createdAt: now(),
      },
      ...s.activity,
    ].slice(0, 20);
    writeStore(s);
    return { sales: s.sales, invoices: s.invoices };
  },
  async quickSell(f) {
    await wait();
    const s = readStore();
    ...
    if (qty <= 0 || qty > Number(p.stock))
      throw new Error('Cantidad inválida o sin stock');
    const total = Number(p.price) * qty;
    const sale = {
      id: `sal_${Date.now()}`,
      unit: p.cat,
      concept: `${p.name} x${qty}`,
      client: f.client || 'Consumidor final',
      amount: total,
      method: f.method || 'Efectivo',
      createdAt: now(),
    };
    ...
    s.products = s.products.map((x) =>
      x.id === p.id ? { ...x, stock: x.stock - qty } : x
    );
    s.activity = [
      {
        id: `act_${Date.now()}`,
        text: `Venta rápida: ${p.name} x${qty}`,
        createdAt: now(),
      },
      ...s.activity,
    ].slice(0, 20);
    writeStore(s);
    return { sales: s.sales, invoices: s.invoices, products: s.products };
  },
  // Costs
  async getCosts() {
    await wait();
    return readStore().costs;
  },
  async saveCost(f) {
    await wait();
    const s = readStore();
    s.costs = f.id
      ? s.costs.map((c) =>
          c.id === f.id ? { ...c, ...f, amount: Number(f.amount) } : c
        )
      : [
          { id: `cst_${Date.now()}`, ...f, amount: Number(f.amount) },
          ...s.costs,
        ];
    writeStore(s);
    return s.costs;
  ...
  async getInvoices(filter = 'Todos') {
    ...
    return filter === 'Todos'
      ? s.invoices
      : s.invoices.filter((i) => i.status === filter);
  },
    ...
    s.invoices = s.invoices.map((i) =>
      i.id === id
        ? {
            ...i,
            status: 'Emitido',
            cae: String(Math.floor(10000000000 + Math.random() * 89999999999)),
            pdfReady: true,
          }
        : i
    );
  ...
  async sendEmail(id) {
    ...
    s.invoices = s.invoices.map((i) => {
      if (i.id !== id) return i;
      if (i.status !== 'Emitido')
        return { ...i, emailStatus: 'Error: no emitida' };
...
function Btn({
  ...
  const base = {
    ...
    ...(disabled || loading ? { opacity: 0.45, pointerEvents: 'none' } : {}),
  };
  const variants = {
  ...
  return (
    ...
    >
      {loading ? (
        <Spinner size={13} color={variant === 'primary' ? '#000' : C.accent} />
      ) : null}
      {children}
    </button>
  );
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: '12px 10px', borderTop: `1px solid ${C.border}` }}>
        ...
        <Btn variant="ghost" size="sm" full onClick={onLogout}>
          Cerrar sesión
        </Btn>
      </div>
    </aside>
...
function LoginView({ onLogin }) {
  ...
  async function handle() {
    ...
    setErr('');
    try {
      await onLogin(form.email, form.pw);
    } catch (e) {
      setErr(e.message);
    } finally {
      setLoading(false);
      ...
      >
        ...
        >
          ...
          <div style={{ position: 'relative' }}>
            ...
            >
              Sistema de gestión para el gimnasio boutique de Bariloche. Socios,
              cobros, stock, caja y facturación ARCA — todo en un lugar.
            </p>
        ...
        >
          ...
          <Btn full loading={loading} onClick={handle}>
            Entrar al sistema
          </Btn>
    ...
    >
      ...
      >
        ...
        <KPI
          ...
          accent={t.profit >= 0 ? C.green : C.red}
        />
        <KPI
          label="Socios activos"
          value={t.activeMembers}
          sub={`${t.expiredMembers} vencidos`}
        />
        <KPI
          label="Transacciones"
          value={t.transactions}
          sub={`Ticket prom: ${money(t.avgTicket)}`}
        />
        <KPI
          label="Stock bajo"
          value={t.lowStock}
          accent={t.lowStock > 0 ? C.amber : C.green}
      ...
      {(t.lowStock > 0 || t.pendingInv > 0 || t.expiredMembers > 0) && (
        ...
        >
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            {t.lowStock > 0 && (
              <Tag color={C.amber}>{t.lowStock} productos con stock bajo</Tag>
            )}
            {t.pendingInv > 0 && (
              <Tag color={C.amber}>{t.pendingInv} facturas pendientes</Tag>
            )}
            {t.expiredMembers > 0 && (
              <Tag color={C.red}>{t.expiredMembers} socios vencidos</Tag>
            )}
          </div>
        </Card>
      )}
...
function CajaView({ sales, invoices }) {
  ...
  const salesHoy = sales.filter((s) =>
    s.createdAt.startsWith(hoy.split('/').reverse().join('/'))
  );
  ...
  const byUnit = {};
  salesHoy.forEach((s) => {
    byUnit[s.unit] = (byUnit[s.unit] || 0) + Number(s.amount);
  });
  const ticket = salesHoy.length ? Math.round(totalHoy / salesHoy.length) : 0;

  const [filtro, setFiltro] = useState(null);
  const timeline = filtro
    ? salesHoy.filter((s) => s.method === filtro)
    : salesHoy;
  ...
  return (
    ...
    >
      ...
      >
        <KPI
          label="Total ingresado hoy"
          value={money(totalHoy)}
          accent={C.green}
        />
        <KPI
          label="Transacciones"
          value={salesHoy.length}
          sub={`Ticket prom: ${money(ticket)}`}
        />
        <KPI
          label="Facturas emitidas"
          value={invoices.filter((i) => i.status === 'Emitido').length}
          accent={C.accent}
          sub={`${
        ...
        <KPI
          ...
          value={money(byMethod['Efectivo'] || 0)}
          accent={C.amber}
        />
      </div>
      ...
      <Card>
        ...
        >
          {Object.entries(byMethod).map(([m, v]) => (
            <button
              key={m}
              onClick={() => setFiltro(filtro === m ? null : m)}
              style={{
                padding: '12px 14px',
                borderRadius: 12,
                textAlign: 'left',
            ...
            >
              <div
                style={{
                  fontSize: 11,
                  color: filtro === m ? C.accent : C.muted,
                  fontWeight: 600,
                  marginBottom: 4,
                }}
              >
        ...
        {filtro && (
          <div style={{ marginTop: 10, fontSize: 12, color: C.accent }}>
            Filtrando por {filtro} — {timeline.length} operaciones ·{' '}
            {money(timeline.reduce((a, s) => a + Number(s.amount), 0))}
          </div>
        )}
      </Card>
      ...
      <Card>
        ...
        {timeline.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
            ...
            >
              {['Hora', 'Socio / Cliente', 'Concepto', 'Método', 'Total'].map(
                (h) => (
                  <div
                    key={h}
                    style={{
            ...
            {timeline.map((s) => (
              ...
              >
                ...
                <div>
                  <Tag color={C.muted2}>{s.method}</Tag>
                </div>
                <div
            ...
            >
              ...
              <div
                style={{ color: C.green, fontWeight: 700, textAlign: 'right' }}
              >
                {money(timeline.reduce((a, s) => a + Number(s.amount), 0))}
              </div>
            </div>
...
function VentaView({ products, onSaleCreated, toast }) {
  ...
  const quickTotal = prod
    ? Number(prod.price) * Number(quick.quantity || 1)
    : 0;
  ...
  return (
    ...
    >
      <div>
        ...
        <div style={{ fontSize: 13, color: C.muted, marginTop: 2 }}>
          Registrá ventas de mostrador y descuentá stock automáticamente
        </div>
      </div>
      ...
      >
        ...
        <Card>
          ...
          >
            <span style={{ color: C.accent }}>⚡</span> Venta rápida · descuenta
            stock
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <Select
              label="Producto"
              value={quick.productId}
              onChange={(v) => setQuick({ ...quick, productId: v })}
              options={[
            ...
            <Input
              ...
              onChange={(v) => setQuick({ ...quick, quantity: v })}
            />
            <Input
              label="Cliente"
              value={quick.client}
              onChange={(v) => setQuick({ ...quick, client: v })}
              placeholder="Consumidor final"
            />
            <Input
              label="Email (para factura)"
              value={quick.email}
              onChange={(v) => setQuick({ ...quick, email: v })}
              type="email"
            />
            <Select
              label="Método de pago"
              value={quick.method}
              onChange={(v) => setQuick({ ...quick, method: v })}
        ...
        <Card>
          ...
          >
            <span style={{ color: C.purple }}>◈</span> Venta manual · membresías
            y servicios
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <Select
              label="Unidad"
              value={manual.unit}
              onChange={(v) => setManual({ ...manual, unit: v })}
              options={[
                'Membresías',
                'Bebidas',
                'Suplementos',
            ...
            <Input
              ...
              onChange={(v) => setManual({ ...manual, concept: v })}
              placeholder="Ej: Plan mensual — Florencia"
            />
            <Input
              label="Cliente"
              value={manual.client}
              onChange={(v) => setManual({ ...manual, client: v })}
            />
            <Input
              label="Email (para factura)"
              value={manual.email}
              onChange={(v) => setManual({ ...manual, email: v })}
              type="email"
            />
            <Input
              label="Monto"
            ...
            <Select
              label="Método de pago"
              value={manual.method}
              onChange={(v) => setManual({ ...manual, method: v })}
              options={['Efectivo', 'Transferencia', 'Débito', 'Crédito', 'QR']}
            />
            <Btn
              full
              ...
              disabled={!manual.concept || !manual.amount}
            >
    ...
    >
      ...
      >
        <div>
          ...
          <div style={{ fontSize: 13, color: C.muted, marginTop: 2 }}>
            ...
            {members.filter((m) => m.status === 'Activa').length} activos
          </div>
        </div>
      ...
      >
        {filtered.map((m) => {
          ...
          const rc =
            { Bajo: C.green, Medio: C.amber, Alto: C.red }[m.risk] || C.muted;
          return (
            <Card key={m.id} style={{ padding: 16 }}>
              ...
              >
                {[
                  ...
                  >
                    ...
                    <div style={{ fontSize: 11, color: C.text, marginTop: 1 }}>
                      {v}
                    </div>
                  </div>
                ))}
              ...
              >
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                  <Btn size="sm" variant="surface" onClick={() => openEdit(m)}>
                    Editar
                  </Btn>
                  <Btn
                    size="sm"
      ...
      <Modal
        open={modal}
        title={form?.id ? 'Editar socio' : 'Nuevo socio'}
        ...
        {form && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            >
              ...
              <Select
                label="Cond. IVA"
                value={form.condIva || 'CF'}
                onChange={(v) => setForm({ ...form, condIva: v })}
                options={[
                  ['CF', 'Consumidor Final'],
                  ['MO', 'Monotributista'],
                  ['RI', 'Resp. Inscripto'],
              ...
              <Input
                label="Vencimiento"
                value={form.renewal || ''}
                onChange={(v) => setForm({ ...form, renewal: v })}
            ...
            >
              ...
              <Btn loading={loading} onClick={handleSave}>
                {form.id ? 'Guardar cambios' : 'Crear socio'}
              </Btn>
            </div>
          </div>
        )}
    ...
    >
      <div>
        ...
        <div style={{ fontSize: 13, color: C.muted, marginTop: 2 }}>
          ...
          {members.filter((m) => m.status === 'Por vencer').length} vencen
          pronto
        </div>
      </div>
      ...
      >
        <KPI
          label="Al día"
          value={members.filter((m) => m.status === 'Activa').length}
          accent={C.green}
        />
        <KPI
          label="Vence pronto"
          value={members.filter((m) => m.status === 'Por vencer').length}
          accent={C.amber}
        />
        <KPI
          label="Vencidos"
          value={members.filter((m) => m.status === 'Vencida').length}
          accent={C.red}
        />
      </div>
        ...
        {modal && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            >
              ...
              <button
                onClick={() => setForm({ ...form, emitir: !form.emitir })}
                style={{
                  width: 40,
                  height: 22,
                  borderRadius: 999,
    ...
    >
      ...
      >
        <div>
          ...
          <div style={{ fontSize: 13, color: C.muted, marginTop: 2 }}>
            {products.filter((p) => p.stock <= p.min).length} productos bajo
            mínimo
          </div>
        </div>
        <Btn
      ...
      >
        {products.map((p) => {
          ...
          return (
            <Card
              key={p.id}
              style={{ border: `1px solid ${bajo ? C.red + '30' : C.border}` }}
            >
              <div
                style={{
                  display: 'flex',
              ...
              >
                ...
                {bajo ? (
                  <Tag color={C.red}>Stock bajo</Tag>
                ) : (
                  <Tag color={C.green}>OK</Tag>
                )}
              </div>
              ...
              {margen !== null && (
                <div style={{ fontSize: 11, color: C.muted, marginBottom: 10 }}>
                  ...
                  <span
                    style={{
                      color:
                        margen > 40 ? C.green : margen > 20 ? C.amber : C.red,
                      fontWeight: 600,
                    }}
                  >
                    {margen}%
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                ...
                <Btn size="sm" variant="danger" onClick={() => onDelete(p.id)}>
                  ×
                </Btn>
              </div>
            </Card>
...
function ArcaView({ invoices, onIssue, onSendEmail }) {
  ...
  const filtered =
    filtro === 'Todos' ? invoices : invoices.filter((i) => i.status === filtro);
  ...
  async function handleIssue(id) {
    ...
    try {
      ...
      setLoading((l) => ({ ...l, [id + '_issue']: false }));
    }
  }
  async function handleEmail(id) {
    ...
    try {
      ...
      setLoading((l) => ({ ...l, [id + '_email']: false }));
    }
  }
  ...
  return (
    ...
    >
      <div>
        ...
        <div style={{ fontSize: 13, color: C.muted, marginTop: 2 }}>
          Comprobantes fiscales · modo mock/homologación activo
        </div>
      </div>
      ...
      >
        ...
        <KPI
          label="Pendientes"
          value={pendientes}
          accent={pendientes > 0 ? C.amber : C.muted}
        />
        <KPI
          label="Mails enviados"
          value={`${emails}/${emitidos}`}
          accent={C.accent}
        />
      </div>
      ...
      <Card>
        {filtered.map((inv, i) => (
          <div
            ...
            style={{
              display: 'grid',
              gridTemplateColumns: '90px 1fr 80px auto',
              gap: 12,
              alignItems: 'center',
              padding: '12px 8px',
              borderBottom:
                i < filtered.length - 1 ? `1px solid ${C.border}22` : 'none',
            }}
          >
            <div
              style={{
            ...
            <div>
              ...
              <div style={{ fontSize: 11, color: C.muted, marginTop: 1 }}>
                ...
                {inv.cae !== '-' && (
                  <span style={{ color: C.green }}> · CAE: {inv.cae}</span>
                )}
              </div>
              <div style={{ fontSize: 10, color: C.muted, marginTop: 1 }}>
      ...
      >
        ...
        >
          ℹ Modo mock · próximos pasos para producción
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {[
            [
              'Obtener certificado AFIP/ARCA',
              'Gestionarlo en AFIP con CUIT del negocio',
            ],
            [
              'Configurar .env',
              'ARCA_CUIT, ARCA_CERT, ARCA_KEY, ARCA_PUNTO_VENTA',
            ],
            [
              'Cambiar adapter a producción',
              'Cambiar ARCA_ENV=produccion en Vercel',
            ],
            [
              'Probar en homologación primero',
              'Con CUIT 20000000001 el adaptador ya funciona',
            ],
          ].map(([t, d]) => (
            <div
              key={t}
...
function CostosView({ costs, sales, onSave, onDelete }) {
  ...
  const totalFijos = costs
    .filter((c) => c.type === 'Fijo')
    .reduce((a, c) => a + Number(c.amount), 0);
  const totalVars = costs
    ...
    >
      ...
      >
        ...
        <KPI
          ...
          value={money(totalVars)}
          accent={C.amber}
        />
        <KPI
          label="Resultado neto"
          value={money(resultado)}
          accent={resultado >= 0 ? C.green : C.red}
        />
      </div>
          ...
          <Card key={tipo}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              ...
              {costs.filter((c) => c.type === tipo).length === 0 && (
                <div
                  style={{ color: C.muted, fontSize: 12, padding: '8px 10px' }}
                >
                  Sin costos {tipo.toLowerCase()}s registrados
                </div>
              )}
              <div
                style={{
      ...
      <Modal
        open={modal}
        title={form?.id ? 'Editar costo' : 'Nuevo costo'}
        onClose={() => setModal(false)}
      >
        {form && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
...
export default function MoveClubApp() {
  ...
  async function handleSaveCost(form) {
    const updated = await api.saveCost(form);
    setCosts(updated);
    show('Costo guardado ✓');
  }
  async function handleDeleteCost(id) {
    await api.deleteCost(id);
    setCosts(readStore().costs);
  ...
  async function handleIssue(id) {
    ...
    setInvoices(updated);
    setActivity(readStore().activity);
    show('Factura emitida (mock ARCA) ✓');
  }
  async function handleSendEmail(id) {
    const updated = await api.sendEmail(id);
    setInvoices(updated);
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', minHeight: '100vh', background: C.bg }}>
        ...
        />
        <main
          style={{ flex: 1, padding: 28, overflowY: 'auto', maxWidth: '100%' }}
        >
